...
//========================================================================
{
	...
	begin();	// add to this widget
	{
		...
		Fl_Button* sinwave = new Fl_Button(605, pty, 80, 20, "sinwave");
		sinwave->callback((Fl_Callback*)Sinwave, this);

		pty += 30;
		
		surfaceBrowser = new Fl_Browser(605, pty, 120, 75, "surface Type");
		surfaceBrowser->type(2);		// select
		surfaceBrowser->callback((Fl_Callback*)damageCB, this);
		surfaceBrowser->add("height map");
		surfaceBrowser->add("sinwave");
		surfaceBrowser->select(1);

		pty += 110;
		/*sinwaveAmp = new Fl_Value_Slider(660, pty, 140, 20, "sin Amp");
		sinwaveAmp->range(0, 10);
		sinwaveAmp->value(1);
		sinwaveAmp->align(FL_ALIGN_LEFT);
		sinwaveAmp->type(FL_HORIZONTAL);

		pty += 30;
		sinwaveLength = new Fl_Value_Slider(660, pty, 140, 20, "sin Len");
		sinwaveLength->range(1, 2.5);
		sinwaveLength->value(2.5);
		sinwaveLength->align(FL_ALIGN_LEFT);
		sinwaveLength->type(FL_HORIZONTAL);*/

		//pty += 30;
		lightButton = new Fl_Button(605, pty, 60, 20, "light");
		togglify(lightButton);
		lightButton->value(1);

		fillButton = new Fl_Button(670, pty, 60, 20, "fill");
		togglify(fillButton);
		fillButton->value(1);
...
//========================================================================
{
	...
	//#####################################################################
	this->trainView->time += speed->value();
	if (this->trainView->time >= 7200)
		this->trainView->time = 0;
	this->trainView->updateClickmap();
	this->trainView->updateSinwavemap();
